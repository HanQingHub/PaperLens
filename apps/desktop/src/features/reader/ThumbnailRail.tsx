...
export default function ThumbnailRail({
  pdf, numPages, currentPage, onGoto,
}: {
    ...
    ;(async () => {
      for (const pageNo of pending) {
        ...
        try {
          ...
          const viewport = page.getViewport({ scale: THUMB_W / vp0.width })
          ...
          if (seqRef.current !== my) return
          cacheRef.current.set(pageNo, canvas.toDataURL('image/webp', 0.7))
  ...
  return (
    <div className="flex shrink-0 flex-col items-center gap-1 overflow-y-auto border-l border-border bg-bg-soft px-1.5 py-2" style={{ width: RAIL_W }}>
      {pages.map((p) => {
        ...
        return (
          <button
            ...
            style={{ width: THUMB_W, height: THUMB_W * 1.3 }}
            title={`第 ${p} 页`}
            onClick={() => onGoto(p)}
          >
            {data ? (
              <img src={data} alt={`第 ${p} 页`} className="h-full w-full object-cover" draggable={false} />
            ) : (
